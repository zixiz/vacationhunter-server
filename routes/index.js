...
router.get("/user", async function(req, res, next) {
  if (req.session.role === "user") {
    ...
    let allVacations = await Vacations.findAll({});
    let followVacationsFromDB = await VacationsOnFollow.findAll({
      where: { username: req.session.username }
    });
    if (followVacationsFromDB.length > 0) {
      for (let i = 0; i < allVacations.length; i++) {
        let check = followVacationsFromDB.find(
          follow => follow.vacationid === allVacations[i].id
        );
        console.log(check);
        if (check === undefined) {
      ...
      res.json({
        vacationsOnFollow: vacationsOnFollow,
        vacationsUnFollow: vacationsUnFollow,
        id: req.session.id,
        role: req.session.role,
        clientName: req.session.clientName,
        userName: req.session.username
      ...
      res.json({
        vacationsOnFollow: vacationsOnFollow,
        vacationsUnFollow: allVacations,
        id: req.session.id,
        role: req.session.role,
        clientName: req.session.clientName,
        userName: req.session.username
